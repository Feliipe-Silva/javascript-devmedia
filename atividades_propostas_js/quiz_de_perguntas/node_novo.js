...
function embaralhar_array( array ) {
    return array.sort(() => Math.random() - 0.5);  // Math.floor(Math.random() * array_nomes.length);
};


function selecionar_pgt_aleatorias() {
    const pgt_embaralhadas = embaralhar_array( array_nomes ); // Aqui abri uma chamada para a função => embaralhar_array
    return pgt_embaralhadas.slice(0, 10);
};


function exibir_perguntas( pontuacao_final ) {
    let colecao_perguntas = selecionar_pgt_aleatorias();
    
    colecao_perguntas.forEach((pergunta, indice) => {
        console.log("Pergunta " + (indice +1) + ': ' + pergunta.pergunta);
...
let pontuacao_final = Number(0);

// const perguntas_selecionadas = selecionar_pgt_aleatorias();
exibir_perguntas();

const resultado_final = exibir_perguntas(pontuacao_final);

console.log("\nJogador(a): " + digite_nome);
console.log("Pontuação final: " + resultado_final + " de 10 Acertos");
console.log("-------------- FIM --------------");